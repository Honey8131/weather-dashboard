$(document).ready(function () {
    ...
    const apiKey ='f6db9f2bf4e517a0b0ee0a1744b00ad1';
    ...
    function searchWeather(queryURL) {
        $.ajax({
            ...
            let forattedDate = moment.unix(respose.dt).format('L');
            ...
            windEl.text((respose.wind.speed * .237).toFixed(1));

            let lat = response.coord.lat;
            let lon = response.coord.lon;
            let queryURLAll = "http://api.openweathermap.org/data/2.5/weather?q=" + city + "&appid=" + APIKey;
            $.ajax({
                url: queryURLAll,
                method: 'GET'
            }).then(function (response) {
